...
# Test 03: Single digit step fails validation
...
cat > "$TEST_ROOT/auth_v1_1/CELL.md" << 'EOF'
---
work_complete: true
---
...
cd "$TEST_ROOT"
set +e
output=$("$CELL_VALIDATE" auth_v1_1 2>&1)
exit_code=$?
set -e
...
if [ $exit_code -ne 1 ]; then
    ...
    cd "$REPO_ROOT"
    rm -rf "$TEST_ROOT/auth_v1_1" "$TEST_ROOT/projectroot.toml"
    exit 1
fi
...
echo "✓ Single digit step fails validation"
exit 0
